...
func TestScaleUpBalanceGroupsRespectsQuota(t *testing.T) {
	tests := []struct {
		...
		// expectedSizes pins the final size of groups whose size is deterministic
		// (e.g. the quota-constrained group). The split of nodes between equally
		// sized, equally constrained groups is arbitrary, so those are not pinned
		// here; expectedTotalAdded checks that the right number of nodes is placed.
		expectedSizes      map[string]int
		expectedTotalAdded int
		// expectAllGroupsGrew asserts every group ended above its initial size,
		// i.e. the remainder was spread across groups rather than dumped into one.
		expectAllGroupsGrew bool
	}{
		{
		...
		},
		{
			...
			// adds at most 1 each; the per-group cap loop then commits the single
			// slot to ng1 (smallest) and caps the rest. ng1: 1→2, ng2/ng3 unchanged.
		...
		},
		{
			...
			// The per-group headroom is computed read-only, so ng2 and ng3 each see
			// the 1 shared slot (over-count); the cap loop then commits quota and
			// caps ng3 to 0 once ng2 consumes the slot. ng1 absorbs more of the
			// request: ng1: 1→5, ng2: 1→2, ng3: 2→2 (5 of 6 nodes placed).
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Iterate in a fixed order; ranging over the map would add the node
			// groups in a random order, making the split of nodes between equally
			// sized groups non-deterministic.
