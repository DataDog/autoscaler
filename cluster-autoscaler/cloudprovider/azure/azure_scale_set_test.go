...
func TestDeleteNodeUnregistered(t *testing.T) {
	...
	manager.azClient.virtualMachineScaleSetsClient = mockVMSSClient
	mockVMSSVMClient := mockvmssvmclient.NewMockInterface(ctrl)
	mockVMSSVMClient.EXPECT().List(gomock.Any(), manager.config.ResourceGroup, "test-asg2", gomock.Any()).Return(expectedVMSSVMs, nil).AnyTimes()
	manager.azClient.virtualMachineScaleSetVMsClient = mockVMSSVMClient
	...
	registered := manager.RegisterAsg(
		newTestScaleSet(manager, "test-asg2"))
	manager.explicitlyConfigured["test-asg2"] = true
	assert.True(t, registered)
	//err = manager.forceRefresh()
	//assert.NoError(t, err)
	manager.regenerateCache()
