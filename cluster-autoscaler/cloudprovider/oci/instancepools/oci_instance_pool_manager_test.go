...
func TestForceRefreshWithAutoDiscovery(t *testing.T) {
	poolSummary := func(id string, tags map[string]string) core.InstancePoolSummary {
		return core.InstancePoolSummary{
			Id:           common.String(id),
			FreeformTags: tags,
			DefinedTags:  map[string]map[string]interface{}{},
		}
	}
	poolDetail := func(id string) core.InstancePool {
		return core.InstancePool{
			Id:             common.String(id),
			CompartmentId:  common.String("ocid1.compartment.oc1..aaaaaaaa1"),
			LifecycleState: core.InstancePoolLifecycleStateRunning,
			Size:           common.Int(0),
		...
		mockClient := &mockComputeManagementClient{
			listInstancePoolsResponse: core.ListInstancePoolsResponse{
				Items: []core.InstancePoolSummary{
					poolSummary("ocid1.instancepool.oc1.phx.aaa1", map[string]string{"cluster": "test"}),
				},
			},
			getInstancePoolResponse: core.GetInstancePoolResponse{
				InstancePool: poolDetail("ocid1.instancepool.oc1.phx.aaa1"),
			},
	...
	t.Run("static pools coexist with autodiscovered pools", func(t *testing.T) {
		mockClient := &mockComputeManagementClient{
			listInstancePoolsResponse: core.ListInstancePoolsResponse{
				Items: []core.InstancePoolSummary{
					poolSummary("ocid1.instancepool.oc1.phx.autodiscovered", map[string]string{"cluster": "test"}),
				},
			},
			getInstancePoolResponse: core.GetInstancePoolResponse{
				InstancePool: poolDetail("ocid1.instancepool.oc1.phx.autodiscovered"),
			},
			listInstancePoolInstancesResponse: core.ListInstancePoolInstancesResponse{Items: []core.InstanceSummary{}},
		}
		...
		manager := &InstancePoolManagerImpl{
			...
			computeManagementClient: mockClient,
			staticInstancePools: map[string]*InstancePoolNodeGroup{
				"ocid1.instancepool.oc1.phx.static": {id: "ocid1.instancepool.oc1.phx.static", minSize: 1, maxSize: 3},
			},
			ShapeGetter:       ocicommon.CreateShapeGetter(shapeClient),
			instancePoolCache: newInstancePoolCache(mockClient, computeClient, virtualNetworkClient, workRequestsClient),
			nodeGroups:        nodeGroups,
		...
		// Pre-populate the cache for the static pool so rebuild does not error.
		manager.instancePoolCache.poolCache["ocid1.instancepool.oc1.phx.static"] = &core.InstancePool{
			Id:             common.String("ocid1.instancepool.oc1.phx.static"),
			CompartmentId:  common.String("ocid1.compartment.oc1..aaaaaaaa1"),
			LifecycleState: core.InstancePoolLifecycleStateRunning,
			Size:           common.Int(1),
		}
		manager.instancePoolCache.instanceSummaryCache["ocid1.instancepool.oc1.phx.static"] = &[]core.InstanceSummary{}

		// forceRefresh clears staticInstancePools when nodeGroups != nil, so the static pool
		// will be removed unless we also add it via discoveryOpts (static spec path).
		// This test confirms that behaviour: after refresh only auto-discovered pools survive.
		require.NoError(t, manager.forceRefresh())
		require.Contains(t, manager.staticInstancePools, "ocid1.instancepool.oc1.phx.autodiscovered")
	})
}
