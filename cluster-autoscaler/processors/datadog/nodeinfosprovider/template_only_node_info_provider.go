...
func (p *TemplateOnlyNodeInfoProvider) SanitizedTemplateNodeInfoFromNodeGroupCached(id string, baseNodeInfo *schedulerframework.NodeInfo,
	...
	podTpls, err := p.podTemplateProcessor.GetDaemonSetPodsFromPodTemplateForNode(baseNodeInfo, taintConfig)
	if err != nil {
		return nil, errors.ToAutoscalerError(errors.InternalError, err)
	}
	for _, pod := range podTpls {
		baseNodeInfo.AddPod(&schedulerframework.PodInfo{Pod: pod})
	}
