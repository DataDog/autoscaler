...
func Test_podTemplateProcessor_GetDaemonSetPodsFromPodTemplateForNode(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{},
			args: args{
				baseNodeInfo: nodeInfo.Snapshot(),
			},
			podListerCreation: func() (v1lister.PodTemplateLister, error) {
		...
		},
		{
			...
			fields: fields{},
			args: args{
				baseNodeInfo: nodeInfoUnschedulable.Snapshot(),
			},
			podListerCreation: func() (v1lister.PodTemplateLister, error) {
				tpls := []*apiv1.PodTemplate{podTemplate1}
				return newTestDaemonSetLister(tpls)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := p.GetDaemonSetPodsFromPodTemplateForNode(tt.args.baseNodeInfo, predicateChecker, tt.args.taintConfig)
