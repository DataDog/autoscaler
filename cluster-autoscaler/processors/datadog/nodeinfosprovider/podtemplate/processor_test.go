...
func Test_podTemplateProcessor_GetDaemonSetPodsFromPodTemplateForNode(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{},
			args: args{
				baseNodeInfo: nodeInfo.DeepCopy(),
			},
			podListerCreation: func() (v1lister.PodTemplateLister, error) {
		...
		},
		{
			...
			fields: fields{},
			args: args{
				baseNodeInfo: nodeInfoUnschedulable.DeepCopy(),
			},
			podListerCreation: func() (v1lister.PodTemplateLister, error) {
				tpls := []*apiv1.PodTemplate{podTemplate1}
				return newTestDaemonSetLister(tpls)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fwHandle, err := schedulerframework.NewTestFrameworkHandle()
			...
			got, err := p.GetDaemonSetPodsFromPodTemplateForNode(tt.args.baseNodeInfo, tt.args.taintConfig)
...
func newNodeInfo(nodeName string, maxPod int64, pods ...*apiv1.Pod) *schedulerframework.NodeInfo {
	...
	podInfos := make([]*schedulerframework.PodInfo, 0, len(pods))
	for _, pod := range pods {
		podInfos = append(podInfos, schedulerframework.NewPodInfo(pod, nil))
	}
	nodeInfo := schedulerframework.NewNodeInfo(node1, nil, podInfos...)
