...
func (p *TemplateOnlyNodeInfoProcessor) Process(ctx *context.AutoscalingContext, daemonsets []*appsv1.DaemonSet, ignoredTaints taints.TaintKeySet) (map[string]*schedulerframework.NodeInfo, error) {
	start := time.Now()
	...
	klog.V(5).Infof("TemplateOnlyNodeInfoProcessor took %s for %d NodeInfos", time.Since(start), len(result))
