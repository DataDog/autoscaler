...
func (p *ScaleDownCandidatesDelayProcessor) GetScaleDownCandidates(ctx *context.AutoscalingContext,
	...
	for _, node := range nodes {
		...
		if !scaleUpTime.IsZero() && scaleUpTime.Add(ctx.ScaleDownDelayAfterAdd).After(currentTime) {
			klog.V(4).Infof("Skipping scale down on node group %s because it scaled up recently at %v", ngID, scaleUpTime)
			continue
		}
		if !scaleDownTime.IsZero() && scaleDownTime.Add(ctx.ScaleDownDelayAfterDelete).After(currentTime) {
			klog.V(4).Infof("Skipping scale down on node group %s because it scaled down recently at %v", ngID, scaleDownTime)
			continue
		}
		if !scaleDownFailureTime.IsZero() && scaleDownFailureTime.Add(ctx.ScaleDownDelayAfterFailure).After(currentTime) {
