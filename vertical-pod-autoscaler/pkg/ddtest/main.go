...
func main() {
	...
	resp, r, err := api.QueryMetrics(ctx, time.Now().Add(-10*time.Second).Unix(), time.Now().Unix(), "kubernetes.cpu.usage.total{kube_cluster_name:snowver}by{kube_namespace,pod_name,container_name}")
