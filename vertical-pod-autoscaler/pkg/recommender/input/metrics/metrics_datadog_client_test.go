...
func TestDdclientPodMetrics_Get(t *testing.T) {
	...
	podMetrics := newDatadogClientWithFactory(time.Second, "test", []string{"env:prod", "team:performance"}, func(config *datadog.Configuration) baseClient {
		return client
	})
