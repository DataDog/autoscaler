...
// integerCPUPostProcessor ensure that the policy is applied to recommendation
// it applies policy for fields: cpuAsInteger
type integerCPUPostProcessor struct{}

const (
	// The user interface for that post processor is an annotation with the following format:
...
func (c integerCPUPostProcessor) Process(vpa *model.Vpa, recommendation *vpa_types.RecommendedPodResources, policy *vpa_types.PodResourcePolicy) *vpa_types.RecommendedPodResources {
