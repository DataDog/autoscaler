...
var (
	...
	excludeUnknownResources = flag.Bool("exclude-unknown-resources", false, "If set to true, the admission controller will not check if any custom resources are able to be scaled and will stick to known resources only")
)
...
func main() {
	...
	controllerFetcher := controllerfetcher.NewControllerFetcher(config, kubeClient, factory, scaleCacheEntryFreshnessTime, scaleCacheEntryLifetime, scaleCacheEntryJitterFactor, *excludeUnknownResources)
